...
func (a *App) PassFlightTest() http.HandlerFunc {
	type request struct {
		RefitID ccc.UUID `json:"refitId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.PassFlightTest, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			default:
				...
				return httpio.NewForbiddenMessagef("PassFlightTest may not run against Refit %s", p.RefitID)
			}
			...
			if err := resources.NewRefitUpdatePatch(p.RefitID).SetStatusID("cleared").Buffer(ctx, txn, resource.UserEvent(ctx)); err != nil {
