...
func (a *App) ClaimMission() http.HandlerFunc {
	type request struct {
		MissionID  ccc.UUID `json:"missionId"`
		SquadronID ccc.UUID `json:"squadronId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.ClaimMission, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			default:
				...
				return httpio.NewForbiddenMessagef("ClaimMission may not run against Mission %s", p.MissionID)
			}
			...
			if err := resources.NewMissionUpdatePatch(p.MissionID).SetStatusID("claimed").Buffer(ctx, txn, resource.UserEvent(ctx)); err != nil {
