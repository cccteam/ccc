...
func (a *App) ScrapShip() http.HandlerFunc {
	type request struct {
		RefitID ccc.UUID `json:"refitId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.ScrapShip, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			case "inspected", "in_refit", "flight_test":
			default:
				// One refusal for the state check and the grant condition
				// alike: the wire never says which said no (§12).
				return httpio.NewForbiddenMessagef("ScrapShip may not run against Refit %s", p.RefitID)
			}
			...
			if err := resources.NewRefitUpdatePatch(p.RefitID).SetStatusID("scrapped").Buffer(ctx, txn, resource.UserEvent(ctx)); err != nil {
