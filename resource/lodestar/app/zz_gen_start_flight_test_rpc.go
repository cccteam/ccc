...
func (a *App) StartFlightTest() http.HandlerFunc {
	type request struct {
		RefitID ccc.UUID `json:"refitId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.StartFlightTest, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			default:
				...
				return httpio.NewForbiddenMessagef("StartFlightTest may not run against Refit %s", p.RefitID)
			}
			...
			if err := resources.NewRefitUpdatePatch(p.RefitID).SetStatusID("flight_test").Buffer(ctx, txn, resource.UserEvent(ctx)); err != nil {
