...
func (a *App) BeginRefit() http.HandlerFunc {
	type request struct {
		RefitID ccc.UUID `json:"refitId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.BeginRefit, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			default:
				...
				return httpio.NewForbiddenMessagef("BeginRefit may not run against Refit %s", p.RefitID)
			}
