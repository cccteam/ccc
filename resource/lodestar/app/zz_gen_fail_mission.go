...
func (a *App) FailMission() http.HandlerFunc {
	type request struct {
		MissionID ccc.UUID `json:"missionId"`
		ReasonID  string   `json:"reasonId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.FailMission, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			default:
				...
				return httpio.NewForbiddenMessagef("FailMission may not run against Mission %s", p.MissionID)
			}
			...
			if err := resources.NewMissionUpdatePatch(p.MissionID).SetStatusID("failed").Buffer(ctx, txn, resource.UserEvent(ctx)); err != nil {
