...
func (a *App) ReplaceMissionDocument() http.HandlerFunc {
	type request struct {
		DocumentID ccc.UUID `json:"documentId"`
	}

	decoder := NewTargetedRPCDecoder[rpc.ReplaceMissionDocument, request](a, accesstypes.Execute)
	...
	return httpio.Log(func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := a.ResourceClient().ExecuteFunc(ctx, func(ctx context.Context, txn resource.ReadWriteTransaction) error {
			...
			row, err := resources.NewMissionDocumentQuery().
				AddColumns(resources.NewMissionDocumentColumns().ID()).
				SetID(p.DocumentID).
				Read(ctx, txn)
			if err != nil {
				return errors.Wrap(err, "resources.MissionDocumentQuery.Read()")
			}
			if row == nil {
				return httpio.NewNotFoundMessagef("MissionDocument %s does not exist", p.DocumentID)
			}
			...
			// NotFound.
			if err := gate.VerifyTenancy(ctx, txn, resource.ExecuteTarget{Resource: "MissionDocuments", Label: "MissionDocument", PKColumn: "Id"}, p.DocumentID); err != nil {
				return err
			}

			// A row-referencing condition on the caller's Execute grant
			// evaluates against the located row, in this transaction.
			if err := gate.Enforce(ctx, txn, resource.ExecuteTarget{Resource: "MissionDocuments", Label: "MissionDocument", PKColumn: "Id"}, p.DocumentID); err != nil {
				return err
			}
