...
// transaction with the session-derived event source; Hangars keep the default and
// write none.
...
func TestChangeTracking(t *testing.T) {
	...
	hangars := accesstypes.Resource("Hangars")
	h := newTestApp(db, grants{
		accesstypes.Create: append(withFields(missionsResource, "clientId", "kindId", "title", "hazard", "fee", "deadline"), withFields(hangars, "name", "zone")...),
	})
	...
	tests := []struct {
		...
		{
			// Hangar is excluded from consolidation, so its standalone PATCH surface
			...
			ops:        `[{"op":"add","path":"/","value":{"name":"Annex Bay","zone":"dock"}}]`,
			table:      "Hangars",
			wantEvents: 0,
		},
	}
