...
| `lead` | Flight Lead Lior, Hammer | `assignedSquadron IN subject.squadrons OR bookedBy = subject`; launch, hold, resume, complete, fail; sorties only while underway. Holds Execute on HoldMission but no Update on the notes, so the armed hold refuses inside the transaction in the grant's words, and CompleteMission's decision-as-data completes without a note. CompleteMission also chooses its status (`@answers(200, 409)`): it answers with the settlement, and a 409 — the booked expenses exceed the fee — carries the same figures with the transaction rolled back. |
