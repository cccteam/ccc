...
// currentUser is a FieldDefaultFunc that stamps the requesting user onto rows the server
// attributes to their author (Mission.BookedBy, DistressCall.FiledBy, Refit.OpenedBy).
// The fields are output_only, so the wire can never supply them: authorship always comes
// from the session, which is also the value grant conditions compare subject against
// (bookedBy = subject, filedBy = subject). The droids outlet binds requests to its
// service identity, so the session is present on every mutation path.
//
// Demonstrates: default_create_fn, output_only, consolidation.batch, workflow.dot, authz-matrix.
...
// defaultCaseNumber issues the server-assigned distress-call case number; the field is
// output_only so a client-supplied value is rejected rather than ignored.
//
// Demonstrates: default_create_fn.
