...
var Demonstrations = []Demonstration{
	...
	{"field.write-only", "A write-only field (FieldMeta.writeOnly) in the browser library: absent from a view and a grid cell, a blank input in create, and in edit over a row read without a capability envelope, whose value travels only when typed, and a list column naming it fails when the page is built, since a list never returns it; a row's Update envelope is planned over the read projection and never names the field, so under one the edit form keeps it absent."},
