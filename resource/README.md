...
| `@order` | `@resource`, `@virtual`, or `@computed` struct | comma list of `Field [asc\|desc]` | Declares the order a list takes when the request carries no `sort`, naming Go fields of the struct; the direction defaults to `asc`. The primary key is appended at runtime so the order is total, and a request's `sort` replaces the declared order for that request. The declaration is optional: a resource that declares nothing and receives no `sort` is not sorted. Its table or view statement carries no `ORDER BY` and the rows arrive in the database's own order, a computed list keeps the order its body yielded, and no cursor is issued, since paging needs an order; `Page-More` marks a first page that did not fit until the request asks a `sort` (section 4). A resource read whole without a sort, by a picker or an export, declares an order or no maximum page size: with a maximum and no order the browser client cannot walk it and refuses rather than answering one unsorted page as every row. A nullable column renders as the plain direction and sorts in its database's own `NULL` placement, so an index on the column serves the order: Spanner places `NULL` first ascending and last descending, PostgreSQL last ascending and first descending. A computed resource follows the same placement: its handler sorts and pages the body's rows where the application's database would. On a computed resource only a leaf field may be named (a nested field is opaque). The generated TypeScript descriptor carries the declared order (`order: [{ field, direction }]`, JSON names), so a browser client knows a request without a `sort` is already ordered and pages by cursor; to a resource that declares none it sends the request as given and shows the first page until the user sorts. On a table-backed resource with a bare `@domain`, generation warns when no index leads with the tenant column and then these columns in this order and direction, naming the index it wants (section 9). Example: [Mission](lodestar/pkg/resources/missions.go). |
...
| `sort` | Comma-separated `field[:direction]` entries, e.g. `name:asc,rank:desc`; direction is `asc` (default) or `desc`. Omitted, the resource's `@order` applies; with neither the list is not sorted (see `cursor`). |
...
the first page asked `count=true`. `Page-More: true` marks a list served with no order
(no `@order`, no `sort`) whose rows did not fit the page; a table statement then
carries no `ORDER BY` and a computed list keeps its body's order, and no cursor is
issued, so paging further requires a sort. `limit=all` answers with none of them. A list served
