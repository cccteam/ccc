...
relations that exist. `limit=all` answers with none of them: the whole list, unsorted
when neither an `@order` nor a `sort` orders it, a table statement then carrying no
`ORDER BY` and a computed list keeping its body's order. A list served cross-origin must
