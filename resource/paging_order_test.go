...
func TestQuerySet_Order(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "a nullable column renders the plain direction ascending: NULL sorts where the database puts it",
			...
			wantSpanner:  "ORDER BY `Note` ASC, `Id` ASC",
			wantPostgres: `ORDER BY "Note" ASC, "Id" ASC`,
		},
		{
			name:         "a nullable column renders the plain direction descending: NULL sorts where the database puts it",
			...
			wantSpanner:  "ORDER BY `Note` DESC, `Name` DESC, `Id` ASC",
			wantPostgres: `ORDER BY "Note" DESC, "Name" DESC, "Id" ASC`,
		},
		{
