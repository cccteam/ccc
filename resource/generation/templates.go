...
import { FieldMeta, Resource, ResourceMap, ResourceMeta{{ if .File.HasNullBoolean }}, NullBoolean{{ end }}{{ if .File.HasCustomTypesInResources }}, CustomTypes{{ end }}, ValidDisplayTypes } from '@cccteam/ccc-lib/types';
...
interface LocalFieldMeta extends Omit<FieldMeta, 'displayType'> {
  displayType: ValidDisplayTypes 
  {{- range $_, $localType := $.LocalTypes }} | '{{ $localType }}'{{- end }}
  {{- if $.File.HasCustomTypesInResources }} | 'customtypes.contentfuldocument'{{ end }};
}
interface LocalResourceMeta extends Omit<ResourceMeta, 'fields'> {
  fields: LocalFieldMeta[];
}

const resourceMap: Record<Resource, LocalResourceMeta> = {
  {{- range $resource := $.Resources }}
