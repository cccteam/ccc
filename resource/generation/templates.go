			...
			for op, err := range resource.Operations(r, "{{ .Resource.OperationPathPattern }}"{{ if not $PrimaryKeyIsGeneratedUUID }}, resource.RequireCreatePath(){{ end }}) {
				...
				{{- if $PrimaryKeyIsGeneratedUUID }}
					...
					if err := patch.PatchSet().Buffer(ctx, txn, eventSource); err != nil {
						return errors.Wrap(err, "resources.{{ .Resource.Name }}CreatePatch.Buffer()")
					}
				...
				{{- if .Resource.HasCompoundPrimaryKey }}
					...
					if err := resources.New{{ .Resource.Name }}UpdatePatchFromPatchSet({{- range $i := .Resource.PrimaryKeys }}id{{ $i }}, {{ end }}patchSet).PatchSet().Buffer(ctx, txn, eventSource); err != nil {
						return errors.Wrap(err, "resources.{{ .Resource.Name }}UpdatePatch.Buffer()")
					}
				{{- else }}
					id := httpio.Param[{{ $PrimaryKeyType }}](op.Req, "id")
				...
				{{- if .Resource.HasCompoundPrimaryKey }}
					...
					if err := resources.New{{ .Resource.Name }}DeletePatchFromPatchSet({{- range $i := .Resource.PrimaryKeys }}id{{ $i }}, {{ end }}patchSet).PatchSet().Buffer(ctx, txn, eventSource); err != nil {
						return errors.Wrap(err, "resources.{{ .Resource.Name }}DeletePatch.Buffer()")
					}
				{{- else }}
					id := httpio.Param[{{ $PrimaryKeyType }}](op.Req, "id")
...
type GeneratedHandlers interface {
	...
	{{ range $resource := .ComputedResources }}
	{{ if not $resource.SuppressListHandler }}{{ Pluralize $resource.Name }}() http.HandlerFunc{{ end }}
	{{ if not $resource.SuppressReadHandler }}{{ $resource.Name }}() http.HandlerFunc{{ end }}
	{{- end }}
}
...
{{ if not $resource.SuppressListHandler }}
	r.Get("/{{ $.RoutePrefix }}/{{ Lower (Kebab (Pluralize $resource.Name)) }}", h.{{ Pluralize $resource.Name }}())
{{- end }}
{{- if not $resource.SuppressReadHandler }}
	{{- if $resource.HasCompoundPrimaryKey }}
	r.Get("/{{ $.RoutePrefix }}/{{ Lower (Kebab (Pluralize $resource.Name)) }}/{{ range $i, $field := $resource.PrimaryKeys }}{{ if $i }}/{{ end }}{{ "{" }}{{ GoCamel $field.Name }}{{ "}" }}{{ end }}", h.{{ $resource.Name }}())
	{{- else if $resource.PrimaryKey }}
	r.Get("/{{ $.RoutePrefix }}/{{ Lower (Kebab (Pluralize $resource.Name)) }}/{{ "{" }}{{ GoCamel $resource.PrimaryKey.Name }}{{ "}" }}", h.{{ $resource.Name }}())
	{{ else }}
	r.Get("/{{ $.RoutePrefix }}/{{ Lower (Kebab (Pluralize $resource.Name)) }}", h.{{ $resource.Name }}())
	{{ end }}
{{- end }}
{{- end }}
}`
