// Package integration drives the waystation's generated HTTP handlers against a real
// Spanner emulator: the structural suites script the permission engine, and the
// conditions suite provisions the shipped demo roles through the real engine, so the
// demo product and the regression suite cannot drift apart.
package integration
