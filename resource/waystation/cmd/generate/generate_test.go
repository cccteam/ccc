...
// generated file differs from the committed state. The committed zz_gen files (Go and
// TypeScript) are the golden output of the resource generators: any drift here is a
// generator behavior change that must be either intentional (commit the new output)
// or a regression.
//
...
func TestGeneratedCodeIsCommitted(t *testing.T) {
	...
	for line := range strings.Lines(string(out)) {
		...
		if strings.Contains(path, "zz_gen") {
