// Package main bootstraps a waystation database in a Spanner emulator for development
...
// universe MigrateRoles reconciles across is read from the seeded Waystations tenant
...
const (
	envProjectID  = "WAYSTATION_SPANNER_PROJECT_ID"
	envInstanceID = "WAYSTATION_SPANNER_INSTANCE_ID"
	envDatabase   = "WAYSTATION_SPANNER_DATABASE"
	envAccessPath = "WAYSTATION_BOOTSTRAP_ACCESS_PATH"
	envDataPath   = "WAYSTATION_BOOTSTRAP_DATA_PATH"

	defaultProjectID  = "waystation-demo"
	defaultInstanceID = "waystation"
	defaultDatabase   = "waystation"
...
// session users and assigns them their roles per domain. The Waystations tenant table
...
// tenantDomains reads the domain universe from the seeded Waystations tenant table.
func tenantDomains(ctx context.Context, spannerClient *spanner.Client) ([]accesstypes.Domain, error) {
	iter := spannerClient.Single().Query(ctx, spanner.Statement{SQL: "SELECT Id FROM Waystations ORDER BY Id"})
	defer iter.Stop()
	...
	if len(domains) == 0 {
		return nil, errors.New("the Waystations table is empty; the demo data must seed the tenants before roles are provisioned")
	}
