...
#                       dev server (`overmind restart console`), which does not watch node_modules
...
case "${1:-}" in
  ...
  push)
    ...
    echo "ccclib.sh: packages pushed; restart the dev server to pick them up: overmind restart console"
    ;;
  restore)
    cd "$GUI_DIR"
    yalc remove --all
