...
// roles files: for every auth the harness provisions, every role at its scope (the global scope, since this application has no tenants),
...
func TestGrants_everyUnconditionalGrantIsServed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(roles.Roles.Domain) > 0 {
				t.Fatalf("%s declares %d domain role(s), but this application has no tenants", tt.rolesPath, len(roles.Roles.Domain))
			}
			for i := range rows {
				row := &rows[i]
