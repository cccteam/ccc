...
func TestLogin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			status, body = b.do(ctx, http.MethodGet, consoleAPI+"/permission-digest?domain="+north, nil)
			if status != http.StatusOK {
			...
			status, body = b.do(ctx, http.MethodGet, consoleAPI+"/tenants/"+north+"/announcements", nil)
			if status != http.StatusOK {
			...
			status, body = b.do(ctx, http.MethodGet, consoleAPI+"/tenants/"+south+"/announcements", nil)
			if status != tt.wantSouth {
				t.Errorf("GET south announcements as %s: status %d, want %d: %s", tt.user, status, tt.wantSouth, body)
			}
			status, body = b.do(ctx, http.MethodGet, consoleAPI+"/tenants/nowhere/announcements", nil)
