...
#                       dev server (`overmind restart portal-web`), which does not watch node_modules
...
case "${1:-}" in
  ...
  push)
    ...
    echo "ccclib.sh: packages pushed; restart the dev server to pick them up: overmind restart portal-web"
    ;;
  restore)
    cd "$GUI_DIR"
    yalc remove --all
