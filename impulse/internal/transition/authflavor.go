...
func (f AuthFlavor) Meaning() string {
	...
	items := []string{
		...
		"Sign in from the browser. The surface's login page becomes a button that sends the browser to `<prefix>/user/login?returnUrl=<page>`, as the reference's portal login component does, in place of the credentials form; a refused login returns to the login page with a code in `?code=`, never text, which the page maps through the library's login messages (`UiCoreService.loginMessage`), and `provideLoginMessages` adds the application's own codes.",
