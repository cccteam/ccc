...
`,
		...
		{
			name: "under the generated router no hand-written file mounts anything",
			files: map[string]string{
				"cmd/generate/main.go": program("pkg/resources",
					`generation.GenerateHandlers("app"),`,
				...
				"pkg/router/zz_gen_router.go":    "package router\n",
				"pkg/resources/announcements.go": outletMembers,
				"web/angular.json":               outletAngularJSON,
				"web/console/proxy.conf.js":      consoleProxy,
				"web/portal/proxy.conf.js":       portalProxy,
		...
		{
			name: "under the generated router a proxy that misses the prefix is still found",
			files: map[string]string{
				"cmd/generate/main.go": program("pkg/resources",
					`generation.GenerateHandlers("app"),`,
					`generation.GenerateRouter(),`,
			...
			wantDetails: []string{"web/portal/proxy.conf.js does not forward /portal/api, the portal outlet's prefix, so ng serve for portal cannot reach it"},
		},
		{
