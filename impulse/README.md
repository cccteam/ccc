...
| `rpc-execute` | Every `@rpc` struct declares `Execute` in one of the two forms the generator classifies by signature (`resource.ReadWriteTransaction` second for the transaction form, `*resource.Client` for the client form; `error` the only or last result), and every generated RPC handler calls it. A handler an older generator could not type-check decodes and returns without running the method. A `TxnRunner` or `DBRunner` interface left in the RPC package warns: the generator reads the signature and no longer consults it, so delete it. |
